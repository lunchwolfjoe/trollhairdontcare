  ...
  const fetchSwapRequestsForFestival = async (festivalId: string) => {
    ...
    try {
      ...
      const processedRequests = requestsData.map(request => {
        ...
        // Find requester
        const requester = volunteersData?.find(v => v.id === request.requester_id) || null;
        
        // Find proposed volunteer
        const proposedVolunteer = volunteersData?.find(v => v.id === request.proposed_volunteer_id) || null;
        ...
        return {
          ...
          requester: {
            id: requester?.id || '',
            full_name: requester?.profiles?.full_name || 'Unknown',
            email: requester?.profiles?.email || ''
          },
          proposed_volunteer: {
            id: proposedVolunteer?.id || '',
            full_name: proposedVolunteer?.profiles?.full_name || 'Unknown',
            email: proposedVolunteer?.profiles?.email || ''
          },
          shift: {
            id: shift?.id || '',
            date: shift?.date || '',
